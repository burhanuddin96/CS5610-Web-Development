defmodule Tasktracker2.Work do
  ...
  def list_tasks_current_user(userID) do
    from(t in Task, where: t.user_id==^userID)
    |>Repo.all()
    |>Repo.preload(:user)
  end
  ...
  def list_half_timeblocks() do
    from(t in TimeBlock, where: is_nil(t.etime))
    |>Repo.all()
    |>Repo.preload(:task)
  end

  def list_task_timeblocks(task_id) do
    from(t in TimeBlock, where: t.task_id == ^task_id)
    |>Repo.all()
    |>Repo.preload(:task)
  end

  def get_half_timeblock_id(task_id) do
    timeblocks = list_half_timeblocks()
    |> recurse_timeblocks(task_id)
  end


  def recurse_timeblocks(timeblocks, task_id) do
    cond do 
      (timeblocks==[]) ->
        ""
      List.first(timeblocks).task_id==task_id ->
        (hd timeblocks).id
      true ->
        recurse_timeblocks(tl(timeblocks),task_id)
    end
  end 
